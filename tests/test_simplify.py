...
def test_qian_qiangua_name_kept_in_t2s():
    ...
    for s in ("乾下乾上", "《乾》：元，亨。", "大哉乾元，万物资始",
              ...
              "乾造：庚申年", "故乾乾因其时而惕", "大哉乾乎", "战乎乾，乾西北之卦也",
              "成象之谓乾，效法之谓坤", "辟户谓之乾", "乾知大始"):
        assert to_simplified(s) == s, f"qián 专名被误转: {s} -> {to_simplified(s)}"
    # gān 义不受保护影响（乾乾 冻结前先把乾乾淨淨 预简化为 干干净净）
    assert to_simplified("乾乾淨淨") == "干干净净"
    assert to_simplified("乾杯") == "干杯"
    # 天干/幹义 正确简体不受锚点误伤
    assert to_simplified("天干地支") == "天干地支"
    assert to_simplified("干，咱也干。不干。") == "干，咱也干。不干。"
    assert to_simplified("晾干，收起来。") == "晾干，收起来。"
