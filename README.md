...
- **英译中翻译（Mac 阅读器，macOS 26.4+）**：英文书章节内一键整章逐段沉浸式对照
  （原文每段下插译文），macOS 系统翻译引擎、完全离线
...
#### 英译中翻译（阅读器，macOS 26.4+）

读英文书（gutenberg / wikisource 英文等）时，章节顶部出现「译」按钮：点一次把当前整章
逐段翻成中文，**译文插在每段原文下方**（沉浸式对照，原文一字不动）；再点「译」恢复纯原文。
简繁切换（中文古籍）与翻译（英文书）互斥出现，各自只在自己适用的书上显示。

- 翻译完全在本地完成（macOS 系统翻译引擎），内容不出本机，无账号、无费用
- 每章只翻一次，结果缓存（`~/.cache/bookfetch/translations/`），反复进出不重翻

**首次使用需要下载系统翻译语言包（约 1GB，一次性）**，两种方式任选：

1. **在 bookfetch 里直接完成（推荐）**：打开一本英文书点「译」→ 提示语言包未装时点「确定」→
   弹出「翻译语言包准备器」→ 点「准备英译中语言包」→ 等它下载安装完成（几分钟）→
   回到阅读器再点「译」即可
...
- [x] N3 翻译（2026-09-05 完成：英译中沉浸式整章对照——macOS 系统翻译桥 swift + 逐段译文 UI；「译」钮仅英文章显示；首次引导「翻译语言包准备器」一键下载+安装语言包约 1GB，全离线）
