...
- `#tab-theme .on`：全局暗色模式开（明/暗持久二态，图标 🌗 恒定、红线标暗色）
- `#reader-toc-toggle .on`：目录面板开
- `#reader-simp .on`：正在读简体（按钮文字=当前语言：繁书初始「繁」，简书初始「简」，点击双向切换）
- `#reader-tr .on`：译文沉浸对照中（再点恢复原文）
- `#reader-theme .on`：阅读器夜间模式开（图标=当前态：日间 ☀️ / 夜间 🌙，红线标夜间）

其它"开关类"新控件照此约定加 `.on`。瞬时动作（字号 Aa、打开设置模态）不加激活态——反馈靠即时效果/浮层本身。
